...
> Full contract: `docs/backend-replacement-api-contract.md` (§1–§5 signatures, §6–§8 below).
...
**Inner product is a contraction, not matrix-multiply.** Add `DDVector::InnerProduct(a, b) = ⟨a|b⟩ = conj(a)·b`. Do **not** pad vectors to matrix form and use `MatrixMultiply` — that is a CFLOBDD artifact. (This is the one function the current 44-test LimTDD build is still missing; it is the 13th `DDVector` function.) *(Implementation note: shipped as a sparse dot product over non-zero amplitudes — see `limtdd-innerproduct-bug-report.md` for why `Package::cont`'s fully-contracted scalar path is not used.)*
